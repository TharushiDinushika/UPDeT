...
try:
    ...
    raise ImportError(
        ...
        "Install it with: pip install pyglet==1.5.27"
    ) from e
...
try:
    ...
    raise ImportError(
        "Error while running 'from pyglet.gl import *'. "
        "Make sure OpenGL is installed. On Ubuntu: apt-get install python-opengl. "
        "On a headless server use: xvfb-run -s '-screen 0 1400x900x24' python script.py"
    ) from e
...
class SimpleImageViewer(object):
    ...
    def imshow(self, arr):
        ...
        assert arr.shape == (self.height, self.width, 3), \
            "You passed in an image with the wrong shape"
